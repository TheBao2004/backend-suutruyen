        ...
        <div class="col-12">
            ...
            <form method="POST" class="bg-secondary rounded h-100 p-4">
                ...
                <div class="d-flex justify-content-between">
                    <div>
                        <h6 class="mb-2">Truyện - {{ $comic->name }}</h6>
                        <h6 class="mb-4">Thêm chương</h6>
                    </div>
                    <div>
                        <a class="btn btn-primary" href="{{ route('admin.chap.index', ['comic' => $comic]) }}">
                            Danh sách
                        </a>
                    </div>
                </div>
                ...
                <div class="row">
                    ...
                    <div class="col-4">
                        <div class="form-floating mb-3">
                            <input type="text" name="stt" class="form-control" placeholder="" value="{{ old('stt') }}">
                            <label>Thứ tự</label>
                            @error('stt')
                                <p class="text-danger mt-2">{{ $message }}</p>
                            @enderror
                ...
                <div class="border border-primary mb-3 p-3" style="border-radius: 5px">
                    ...
                    <div class="box_drag_grop">
                        ...
                        @if (old('path_image_chap_comic'))

                            @foreach (old('path_image_chap_comic') as $d)
                                <div class="item_drag_grop position-relative">
                                    <div class="item_image_drag_grop" style="background-image: url('{{'http://127.0.0.1:8000/storage/'.$d}}')"></div>
                                    <input type="hidden" value="{{$d}}" name="path_image_chap_comic[]">
                                    <span class="delete_item_drag_grop position-absolute top-0 start-100 translate-middle badge rounded-pill" style="cursor: pointer;">
                                        <i class="fa fa-times"></i>
                                    </span>
                                </div>
                ...
                <div class="form-group mb-3">
                    ...
                    <textarea name="content" class="form-control ckeditor" id="" cols="30" rows="10">{{ old('content') }}</textarea>
                    @error('content')
                    <p class="text-danger mt-2">{{ $message }}</p>
                    @enderror
                </div>
